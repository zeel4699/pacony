...
class AppServiceProvider extends ServiceProvider
{
    ...
    public function boot()
    {
        ...
        if (Schema::hasTable('order_modes')) {
            ...
            View::share('general_order_mode', $general_order_mode);

        }

        // The default value is assigned.
        $general_unread_demo_requests_count = [];
        View::share('general_unread_demo_requests_count', $general_unread_demo_requests_count);
        if (isset($general_order_mode)){
            if ($general_order_mode->order_mode == "with_free_trial") {

                if (Schema::hasTable('demo_requests')) {
                    // Retrieve messages
                    $general_unread_demo_requests_count = DemoRequest::where('read', 0)->get();
                    View::share('general_unread_demo_requests_count', $general_unread_demo_requests_count);
                }

            } elseif ($general_order_mode->order_mode == "via_whatsapp") {

                if (Schema::hasTable('order_via_whatsapps')) {
                    // Retrieve messages
                    $general_unread_demo_requests_count = OrderViaWhatsapp::where('read', 0)->get();
                    View::share('general_unread_demo_requests_count', $general_unread_demo_requests_count);
                }
